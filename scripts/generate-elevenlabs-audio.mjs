...
fs.writeFileSync(
  path.resolve(audioDir, "manifest.json"),
  JSON.stringify(manifest, null, 2),
  "utf8",
);
console.log(
  `Generated audio manifest with ${manifest.length} entries at assets/audio/manifest.json`,
);

if (!apiKey) {
  console.log(
    "ℹ️ No ElevenLabs API key provided. Web Speech API fallback will handle live narration.",
  );
  console.log(
    "To generate offline MP3s in batch, run: node scripts/generate-elevenlabs-audio.mjs --key=YOUR_KEY",
  );
  process.exit(0);
}

console.log(
  `Starting ElevenLabs audio batch generation using voice ID: ${voiceId}...`,
);
